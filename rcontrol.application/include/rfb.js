...
var that           = {},  // Public API methods
    ...
    rfb_version    = 0,
    // Include latest vnc version. Shylendra
    rfb_max_version= 3.8,
    rfb_auth_scheme= '',
   ...
   // scan_imgQ_rate = 100,
    scan_imgQ_rate = 20, // 25 times per second or so
    last_req_time  = 0,
    rre_chunk_sz   = 100,
...
function constructor() {
    ...
    console.log("encodings.length : " + encodings.length);
    ...
    try {
        ...
        Util.Debug("Display exception: " + exc);
    ...
    ws.on('open', function() {
        if (rfb_state === "connect") {
            ...
            //enyo.windows.addBannerMessage("Got unexpected WebSockets connection", "{}");
            fail("Got unexpected WebSockets connection");
        }
    });
    ws.on('close', function() {
    	console.log("in close para: " + rfb_state);
        if (rfb_state === 'disconnect') {
            console.log("going thru disc.");
            updateState('disconnected', 'VNC disconnected');
        } else if (rfb_state === 'ProtocolVersion') {
            //enyo.windows.addBannerMessage("Failed to connect to server", "{}");
            fail('Failed to connect to server');
        } else if (rfb_state in {'failed':1, 'disconnected':1}) {
            Util.Error("Received onclose while disconnected");
        } else  {
            //enyo.windows.addBannerMessage("Server disconnected", "{}");
            fail('Server disconnected');
        }
    });
    ws.on('error', function(e) {
        //enyo.windows.addBannerMessage("WebSocket error : " + e, "{}");
        fail("WebSock error: " + e);
        Util.Debug("WebSock error: " + e);
    });
    ...
    //if (Websock_native) {
        ...
        console.log("Time Out:" + def_con_timeout);
...
updateState = function(state, statusMsg) {
     ...
     */
    if (state in {'disconnected':1, 'loaded':1, 'connect':1, // (Shylendra) 
        ...
        if (display && display.get_context()) {
            ...
            if ((Util.get_logging() !== 'debug') ||
                ...
                // debug is off
                display.clear();
                var b_canvas = document.getElementById("VNC_canvas");
                b_canvas.width = 1024;
                b_canvas.height = 700;
                var b_context = b_canvas.getContext("2d");
                b_context.fillStyle = "rgb(238,238,238)";
                b_context.fillRect(0, 0, 1024, 700);
            }
        }
    ...
    case 'connect':
        ...
        connTimer = setTimeout(function () {
                //enyo.windows.addBannerMessage("Connection timeout!", "{}");
                fail("Connect timeout");
            }, conf.connectTimeout * 1000);
//            }, conf.connectTimeout * 1000); Shylendra
    ...
    case 'disconnect':
        ...
        if (! test_mode) {
            disconnTimer = setTimeout(function () {
                    //enyo.windows.addBannerMessage("Disconnect timeout", "{}");
        ...
        if (normal_msg() && ws.rQlen() > 0) {
            ...
            if (msgTimer === null) {
                ...
                msgTimer = setTimeout(function () {
                            ...
                            handle_message();
                            console.log("after handle message in timer loop");
                        }, 100);
            } else {
...
mouseButton = function(x, y, down, bmask) {
    console.log("Called -- mouseButton");
    if (down) {
...
init_msg = function() {
    ...
    case 'ProtocolVersion' :
        if (ws.rQlen() < 12) {
            //enyo.windows.addBannerMessage("Incomplete protocol version", "{}");
        ...
        switch (sversion) {
            ...
            case "003.889": rfb_version = 3.3; break; // Mac OSX/Lion (Even with this, there are issues.
            case "003.007": rfb_version = 3.7; break;
            case "003.008": rfb_version = 3.8; break;
            case "004.001": rfb_version = 3.8; break; // RealVNC Enterprise Edition.(Even with this, there are issues.
            default:
                //enyo.windows.addBannerMessage("Invalid server version : " + sversion, "{}");
    ...
    case 'Security' :
        if (rfb_version >= 3.7) {
            ...
            if (num_types === 0) {
                ...
                //enyo.windows.addBannerMessage("Security Failure: " + reason, "{}");
            ...
            if (rfb_auth_scheme === 0) {
                //enyo.windows.addBannerMessage("Unsupported security types:" + types, "{}");
    ...
    case 'Authentication' :
        ...
        switch (rfb_auth_scheme) {
            case 0:  // connection failed
                ...
                strlen = ws.rQshift32();
                reason = ws.rQshiftStr(strlen);
                //enyo.windows.addBannerMessage("Auth failure: " + reason, "{}");
            ...
            case 2:  // VNC authentication
                ...
                return;
            default:
                //enyo.windows.addBannerMessage("Unsupported auth scheme: " + rfb_auth_scheme, "{}");
        ...
        switch (ws.rQshift32()) {
            ...
            case 1:  // failed
                if (rfb_version >= 3.8) {
                    ...
                    //enyo.windows.addBannerMessage(reason, "{}");
                    fail(reason);
                } else {
                    //enyo.windows.addBannerMessage("Authentication Failed.", "{}");
                    console.log("Banner Message Done");
                    fail("Authentication failed");
                }
                return;
            case 2:  // too-many
                //enyo.windows.addBannerMessage("Too many auth attempts", "{}");
...
normal_msg = function() {
    ...
    case 3:  // ServerCutText
        ...
        break;
    default:
        //enyo.windows.addBannerMessage("Disconnected: illegal server message type : " + msg_type, "{}");
...
framebufferUpdate = function() {
    ...
    //console.log("FBU.rects FBU.rects FBU.rects FBU.rects : " + FBU.rects + " State : " + rfb_state);
    while (FBU.rects > 0) {
        if (rfb_state !== "normal") {
            return false;
        }
        ...
        if (FBU.bytes === 0) {
            ...
            if (encNames[FBU.encoding]) {
                ...
                */
                ...
                //enyo.windows.addBannerMessage("Disconnected", "{}");
...
pointerEvent = function(x, y) {
    ...
    //console.log("ivide aano" + mouse_buttonMask + "X:" + x + "Y:" + y);
   // var ttt = document.getElementById("VNC_canvas");
   /* console.log(ttt.clientHeight);
    console.log(ttt.clientLeft);
    console.log(ttt.clientTop);
    console.log(ttt.clientWidth);
    console.log(ttt.offsetHeight);
    console.log(ttt.offsetLeft);
    console.log(ttt.offsetTop);
    console.log(ttt.offsetWidth);
    console.log(ttt.scrollHeight);
    console.log(ttt.scrollLeft);
    console.log(ttt.scrollTop);
    console.log(ttt.scrollWidth);*/
    var arr;
    arr = [5];  // msg-type
...
that.sendEnter = function() {
    ...
    var arr = [];
    arr = arr.concat(keyEvent(0xFF0D, 1)); // Enter
    arr = arr.concat(keyEvent(0xFF0D, 0)); // Enter
    arr = arr.concat(fbUpdateRequest(1));
    ws.send(arr);
};

that.sendEsc = function() {
    if (rfb_state !== "normal") { return false; }
    Util.Info("Sending Esc");
    var arr = [];
...
that.sendPageUp = function() {
    if (rfb_state !== "normal") { return false; }
    Util.Info("Sending Page Up");
    var arr = [];
    arr = arr.concat(keyEvent(0xFF55, 1)); // Page Up
    arr = arr.concat(keyEvent(0xFF55, 0)); // Delete
    arr = arr.concat(fbUpdateRequest(1));
    ws.send(arr);
};


that.sendPageDown = function() {
    if (rfb_state !== "normal") { return false; }
    Util.Info("Sending Page Up");
    var arr = [];
    arr = arr.concat(keyEvent(0xFF56, 1)); // Page Up
    arr = arr.concat(keyEvent(0xFF56, 0)); // Delete
    arr = arr.concat(fbUpdateRequest(1));
    ws.send(arr);
};
